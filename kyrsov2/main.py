...
def peresech(lin,obl):#определяет пересекает ли область линии
    ...
    for i in range(0,len(lin)):
         ...
         if (obl[0][0]>=lin[i][0] or obl[3][0]>=lin[i][0]) and (obl[1][0])<=lin[i][2]:#по крайним границам линии по х
             if (obl[0][1] <= lin[i][3] and (obl[2][1]) >= lin[i][1]) or (obl[0][1] >= lin[i][3] and (obl[2][1]) <= lin[i][1]):  # по крайним границам линии по у надо доделать
                k=i
                break
    #print(k)
    return k
...
while video.isOpened():
    ...
    #houghlinesP
    linesP=cv2.HoughLinesP(tresh,1,np.pi/180,50,np.array([()]),60,10)
    j=0
    lenese=[]
    ...
    for i in range(1, len(lenese)):
        l=lenese[i]
        #l1=lenese1[i]
        if abs(lback[0]-l[0])>100 and abs(lback[2]-l[2])>100 and abs(lback[1]-l[1])>10: #and abs(l[4]-lback[4])>0.004:
            j+=1
            ...
            #cv2.putText(rgb_image, str(k), (l[0], l[1]), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            linline.append([0] * 6)
            linline[j - 1][0] = l[0]
            linline[j - 1][1] = l[1]
            linline[j - 1][2] = l[2]
            ...
            linline[j - 1][5] = l[4]
            lback=l
            #print(linline[j-1],j-1)
    i=1
    k=0
    while i <len(linline):
        l=linline[i]
        if l[5]==-0.567 or l[5]==-0.364  or l[5]==0.6 or l[5]==0.344 or l[5]==1.757 or l[5]>2:
                linline.remove(l)
        else:
            for q in range(0,i):
                lback=linline[q]
                if abs(lback[5]-l[5])<=50 and abs(lback[0]-l[0])<=10 and abs(lback[2]-l[2])<=10 and abs(lback[1]-l[1])<=10:
                    linline.remove(l)
                #lenlin -= 1
                    break
        i+=1
    ...
    """
            [424, 561, 469, 487, 0, -1.644] 2-
            [384, 613, 513, 407, 0, -1.597] 4
            
            [204, 404, 315, 612, 0, 1.874]  16
            [200, 407, 310, 615, 0, 1.891] 18-
            
            [385, 746, 534, 1026, 0, 1.879] 17
            [381, 748, 529, 1028, 0, 1.892]  19-
            
            --
            [162, 428, 222, 394, 0, -0.567] 5
            [396, 893, 462, 869, 0, -0.364] 8
            [403, 491, 464, 512, 0, 0.344] 10
            [61, 998, 126, 1037, 0, 0.6] 12
            [391, 603, 428, 668, 0, 1.757] 15
    """
    """ for i in range(1, len(linline)):
            l=lenese[i]
            for k in range(0,j):
                lback=lenese[k]
                if (abs(lback[0]-l[0])>100 and abs(lback[2]-l[2])>100 and abs(lback[1]-l[1])>10):
...
for l in linline:
    k += 1
    print(l, k)
linline.remove(linline[11])
linline.remove(linline[7])
linline.remove(linline[2])
linline.remove(linline[1])
k=0
for l in linline:
    k += 1
    print(l, k)
"""
[358, 467, 361, 402, 0, -21.667] 1
[384, 613, 513, 407, 0, -1.597] 2
[526, 580, 645, 514, 0, -0.555] 3
[33, 853, 141, 795, 0, -0.537] 4
[67, 676, 237, 678, 0, 0.012] 5
[528, 793, 661, 873, 0, 0.602] 6
[96, 529, 175, 578, 0, 0.62] 7
[204, 404, 315, 612, 0, 1.874] 8
[381, 748, 529, 1028, 0, 1.892] 9

симметричные пары
[204, 404, 315, 612, 0, 1.874] 8
[381, 748, 529, 1028, 0, 1.892] 9 Большой круг

[528, 793, 661, 873, 0, 0.602] 6
[96, 529, 175, 578, 0, 0.62] 7   маленький круг
...
отобразить
[358, 467, 361, 402, 0, -21.667] 1
[384, 613, 513, 407, 0, -1.597] 2  [526, 580, 645, 514, 0, -0.555] 3 [33, 853, 141, 795, 0, -0.537] 4
[67, 676, 237, 678, 0, 0.012] 5
